...
def main():
    ...
    # Upload style image
    style_pic = st.file_uploader("Upload Style Image", type=["jpg", "jpeg", "png"])
    if style_pic is not None:
        style_image = Image.open(style_pic)
        st.image(style_image, caption='Style Image', use_column_width=True)
        style = loader(style_image).unsqueeze(0).to(device, torch.float)

    # Upload content image
    content_pic = st.file_uploader("Upload Content Image", type=["jpg", "jpeg", "png"])
    if content_pic is not None:
        content_image = Image.open(content_pic)
        st.image(content_image, caption='Content Image', use_column_width=True)
        content = loader(content_image).unsqueeze(0).to(device, torch.float)
        
    if style_pic is not None and content_pic is not None:
        input_img = content.clone()
        
        with st.spinner('Processing...'):
            output = run_style_transfer(model, model_mean, model_std, content, style, input_img)
        
        st.image(output.cpu().squeeze(0).permute(1, 2, 0).clamp_(0, 1).numpy(), caption='Output Image', use_column_width=True)

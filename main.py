...
model_mean = torch.tensor([0.485, 0.456, 0.406]).to(device)
model_std = torch.tensor([0.229, 0.224, 0.225]).to(device)
...
if content_image_file and style_image_file:
    ...
    if st.button('Run Style Transfer'):
        output = run_style_transfer(model, model_mean, model_std, content_tensor, style_tensor, input_img)

        st.write("Generated Image:")
        st.image(output.cpu().squeeze(0).permute(1, 2, 0).numpy(), caption='Output Image', use_column_width=True)
